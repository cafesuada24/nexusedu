...
class SQLWorkerNode:
    ...
    def __call__(self, state: SQLTask, config: RunnableConfig) -> dict[str, Any]:
        ...
        for i in range(MAX_LOOP):
            raw_data, current_hint = self._attempt_sql_execution(
            ...
            if raw_data is not None and (not isinstance(raw_data, list) or not raw_data or 'error' not in raw_data[0]):
                break
        else:
            return {
                'results': [
    ...
    def _attempt_sql_execution(  # noqa: PLR0913
        self,
        *,
        ...
        try:
            logger.info(f'SQL_Worker [{db_id}]: Executing query (Attempt {attempt + 1})...')
