"""Application service for providing database metadata for agent tasks."""
...
class AgentMetadataService:
    ...
    async def get_formatted_db_list(self) -> str:
        """Retrieve list of available databases in markdown format."""
        header = '## AVAILABLE DATABASE REGISTRY\n'
        entries: list[str] = []
        registry = await self.metadata_repo.get_db_registry()
        for db in registry:
