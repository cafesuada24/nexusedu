...
MAJORS = ['Computer Science', 'Data Science', 'Mathematics', 'Physics', 'Software Engineering']
COURSE_NAMES = [
...
def generate_mock_data():
    ...
    for i in range(NUM_STUDENTS):
        sid = f"S{1000 + i}"
        name = f"Student {i}"
        profile = random.choices(['steady', 'improving', 'degrading'], weights=[0.6, 0.2, 0.2])[0]
        students.append({
            'sid': sid,
            'student_name': name,
    ...
    courses = [{'id': f"C{100 + i}", 'name': name} for i, name in enumerate(COURSE_NAMES)]
                    ...
                    for week in range(1, WEEKS_PER_SEMESTER + 1):
                        ...
                        if random.random() > 0.4:  # 60% chance of an activity each week
                            ...
                            if profile == 'degrading':
                                base_score = 90 - (year * 10) - (week * 0.5) + random.randint(-5, 5)
                            elif profile == 'improving':
                                base_score = 60 + (year * 8) + (week * 0.3) + random.randint(-5, 5)
                            else:
                            ...
                            activities.append({
                                ...
                                'timestamp': float(base_timestamp + semester_offset + (week * 7 * 24 * 3600) + random.randint(0, 86400)),
                                'academic_year': year,
                                'semester': semester,
                                'week': week
                            })
...
if __name__ == "__main__":
    ...
    legacy_df = df_activities.merge(df_students[['sid', 'student_name', 'email']], on='sid', how='left')
