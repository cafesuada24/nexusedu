...
import { Sparkles, Send, RotateCcw, CalendarDays, ExternalLink, Mail, User, BookOpen } from "lucide-react"
import {
  Sheet,
  SheetContent,
  SheetDescription,
...
export function EmailEditorSheet({ alert, onClose, onSave }: Props) {
  const { data: draft } = useDraftStatus(alert?.id)
  ...
  React.useEffect(() => {
    if (alert) {
      ...
      const currentSubject = subject || alert.subject || alert.draftSubject || draft?.subject || ""
      const currentBody = body || alert.body || alert.draftBody || draft?.body || ""
  ...
  const isGenerating = draft?.is_generating ?? (!!alert?.draftJobId && !alert?.draftBody)

  return (
    <Sheet open={!!alert} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="flex w-full flex-col p-0 sm:max-w-[700px]">
        <SheetHeader className="px-6 pt-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <SheetTitle className="font-serif text-xl">Soạn thảo Email</SheetTitle>
                <SheetDescription>
            ...
            <Badge
              ...
              className={cn(
                ...
                isGenerating ? "bg-primary/15 text-primary" : (body ? "bg-success/15 text-success" : "bg-muted text-muted-foreground")
              )}
            >
              <Sparkles className="mr-1 size-3" />
              {isGenerating ? "AI đang soạn..." : (body ? "Bản nháp đã sẵn sàng" : "Chưa có bản nháp")}
            </Badge>
          </div>
        </SheetHeader>
        ...
        <ScrollArea className="flex-1 px-6">
          <div className="grid gap-6 py-6">
            ...
            <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border/60 bg-muted/30 p-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-muted-foreground">MSSV: {alert?.mssv}</span>
              </div>
              <Separator orientation="vertical" className="h-4" />
              <div className="flex items-center gap-2">
            ...
            <div className="grid gap-3">
              <Label htmlFor="subject" className="text-sm font-semibold">Tiêu đề Email</Label>
              <Input
                id="subject"
                value={subject}
            ...
            <div className="grid gap-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="body" className="text-sm font-semibold">Nội dung chi tiết</Label>
                <span className="text-[11px] text-muted-foreground">
                  {body.split(/\s+/).filter(Boolean).length} từ
                </span>
              </div>
              <Textarea
                id="body"
                ...
                placeholder={isGenerating ? "Hệ thống đang phân tích kết quả học tập để soạn thư hỗ trợ phù hợp..." : "Nhập nội dung email tại đây..."}
              ...
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                <p className="text-[11px] text-muted-foreground italic">
                  Mẹo: Nên bắt đầu bằng lời hỏi thăm chân thành trước khi đề cập đến kết quả học tập.
                </p>
                <Button 
                  variant="ghost" 
                  size="sm" 
            ...
            <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-1">
                  <p className="font-semibold text-primary">Đính kèm liên kết đặt lịch</p>
                  <p className="text-sm leading-snug text-muted-foreground">
                    Sinh viên sẽ nhận được liên kết này để chủ động chọn lịch hẹn tư vấn với bạn.
                  </p>
                  <div className="pt-2">
                    <Link
                      href="/booking/le-ha"
        ...
        <SheetFooter className="gap-3 p-6 sm:justify-end">
          <Button 
            variant="outline" 
          ...
          <Button
            ...
            onClick={() => alert && onSave({ ...alert, subject, body })}
          >
