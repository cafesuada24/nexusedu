...
if TYPE_CHECKING:
    ...
    from src.domain.ports.repositories import StudentRepository, EmailRepository
...
async def test_get_alerts(
    ...
    await student_repository.ingest_students([
        {'sid': 'A1', 'student_name': 'Alerted', 'email': 'a@ex.com', 'intervention_status': 'new'},
        {'sid': 'N1', 'student_name': 'Normal', 'email': 'n@ex.com', 'intervention_status': 'none'},
    ])
...
async def test_update_alert_status(
    ...
    await student_repository.ingest_students([
        {'sid': 'P1', 'student_name': 'P', 'email': 'p@ex.com', 'intervention_status': 'new'},
    ])
    await student_repository.session.commit()
...
@pytest.mark.asyncio
async def test_trigger_draft(client: TestClient, student_repository: StudentRepository) -> None:
    """Verify manual draft triggering."""
    await student_repository.ingest_students([
        {'sid': 'T1', 'student_name': 'T', 'email': 't@ex.com', 'intervention_status': 'new'},
    ])
    await student_repository.session.commit()
...
@pytest.mark.asyncio
async def test_review_draft_idempotency(client: TestClient, student_repository: StudentRepository) -> None:
    """Verify points awarding and idempotency for draft review."""
    sid = 'R1'
    await student_repository.ingest_students([{'sid': sid, 'student_name': 'R', 'email': 'r@ex.com'}])
    await student_repository.session.commit()
...
@pytest.mark.asyncio
async def test_send_email_flow(client: TestClient, student_repository: StudentRepository, email_repository: EmailRepository) -> None:
    """Verify the full email sending flow."""
    sid = 'S1'
    await student_repository.ingest_students([{'sid': sid, 'student_name': 'S', 'email': 's@ex.com', 'intervention_status': 'new'}])
...
@pytest.mark.asyncio
async def test_get_email_draft(client: TestClient, student_repository: StudentRepository, email_repository: EmailRepository) -> None:
    """Verify retrieving the current draft content."""
    sid = 'D1'
    await student_repository.ingest_students([{'sid': sid, 'student_name': 'D', 'email': 'd@ex.com'}])
