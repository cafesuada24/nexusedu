...
@pytest.fixture
def raw_client():
    """Provides a clean TestClient."""
    app.dependency_overrides.clear()
    with TestClient(app) as c:
        yield c
    app.dependency_overrides.clear()

def test_admin_promote_user(raw_client: TestClient):
    """Verify that an admin can promote another user's role."""
    # 1. Register a regular user
    user_email = f"user_{uuid.uuid4().hex[:8]}@example.com"
    user_password = "password123"
    raw_client.post("/api/v1/auth/register", json={"email": user_email, "password": user_password})
    ...
    # Login as User to get their own ID from /me
    user_login = raw_client.post("/api/v1/auth/jwt/login", data={"username": user_email, "password": user_password})
    user_token = user_login.json()["access_token"]
    me_resp = raw_client.get("/api/v1/users/me", headers={"Authorization": f"Bearer {user_token}"})
...
def test_non_admin_cannot_promote(raw_client: TestClient):
    ...
    raw_client.post("/api/v1/auth/register", json={"email": user1_email, "password": "password"})
    
    user2_email = f"u2_{uuid.uuid4().hex[:8]}@example.com"
    raw_client.post("/api/v1/auth/register", json={"email": user2_email, "password": "password"})

    # 2. Login as User 1
    login_resp = raw_client.post("/api/v1/auth/jwt/login", data={"username": user1_email, "password": "password"})
